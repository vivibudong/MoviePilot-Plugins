...
class DoubanHaixiangkan(_PluginBase):
    ...
    plugin_name = "豆瓣还想看"
    # 插件描述
    plugin_desc = "豆瓣想看修改版，可重复入库已经删除源文件的历史想看。"
    # 插件图标
            ...
            for result in results:
                try:
                    ...
                    if douban_id in [h.get("doubanid") for h in history]:
                        logger.info(f'标题:{title},豆瓣ID:{douban_id} 历史记录中已存在,检查媒体库状态...')
                        # 先识别媒体信息
                        meta_check = MetaInfo(title=title)
                        douban_info_check = self.chain.douban_info(doubanid=douban_id)
                        meta_check.type = MediaType.MOVIE if douban_info_check.get("type") == "movie" else MediaType.TV
                        
                        if settings.RECOGNIZE_SOURCE == "themoviedb":
                            tmdbinfo_check = mediachain.get_tmdbinfo_by_doubanid(doubanid=douban_id, mtype=meta_check.type)
                            if tmdbinfo_check:
                                mediainfo_check = self.chain.recognize_media(meta=meta_check, tmdbid=tmdbinfo_check.get("id"))
                            else:
                                mediainfo_check = None
                        else:
                            mediainfo_check = self.chain.recognize_media(meta=meta_check, doubanid=douban_id)
                        
                        # 如果能识别到媒体信息,检查是否在媒体库中存在
                        if mediainfo_check:
                            exist_flag_check, _ = downloadchain.get_no_exists_info(meta=meta_check, mediainfo=mediainfo_check)
                            if exist_flag_check:
                                logger.info(f'标题:{title},豆瓣ID:{douban_id} 已处理过且媒体库中存在,跳过')
                                continue
                            else:
                                logger.info(f'标题:{title},豆瓣ID:{douban_id} 已处理过但媒体库中不存在,重新处理')
                                # 从历史记录中删除该条记录,以便重新处理
                                history = [h for h in history if h.get("doubanid") != douban_id]
                        else:
                            logger.info(f'标题:{title},豆瓣ID:{douban_id} 无法识别媒体信息,跳过')
                            continue
